"""
Approccio Q-Learning/Genetico

L'agente impara con il Q-Learning per num_training_episodes,
...
for gen in range(num_generation):
    ...
    for agent in range(population_size):
        ...
        for ep in range(num_training_episodes):
            ...
            while not done:
                ...
                next_state, rew, done, info = env.step(action) # Take one step in the environment

                # il Q-learning aggiorna il valore state-action (prende in considerazione il massimo valore Q per il prossimo stato)
                Q[agent, state, action] = Q[agent, state, action] * (1 - learning_rate) + learning_rate * (rew + discount_rate * np.max(Q[agent,next_state, :]))
    ...
    # si effettua una ricerca del massimo per trovare il tasso di success maggiore e la generazione corrispondente
    maxx=0
